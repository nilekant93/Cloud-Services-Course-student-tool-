  ...
  const testLesson = async () => {
    ...
    try {
      ...
      if (res.ok && data.test_passed) {
        ...
        localStorage.setItem('week4Tested', 'true');
        await markWeekDone();
      } else {
        setTestStatus('failed');
      }
  ...
  const markWeekDone = async () => {
    ...
    try {
      const res = await fetch('http://localhost:3001/mark_week_done', {
        ...
        body: JSON.stringify({ week: 'week4done' }),
      });
    ...
    <Layout>
      <div className="container mx-auto max-w-4xl">
        <div className="space-y-6">
          <h1 className="text-3xl md:text-4xl font-bold tracking-tight">Week 4</h1>
          <p className="text-lg text-muted-foreground">
            Hosting with Firebase and checking your deployment
          </p>
            ...
            <CardContent>
              ...
              <div className="space-y-4">
                <div className="flex flex-col md:flex-row gap-4">
                  <Input
                    placeholder="Enter your Firebase hosted URL"
                    value={url}
